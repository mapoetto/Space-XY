...
public class PoolThread {
	...
	*/
	...
	public synchronized static void notifyMyPresence(ServerThread st) {
		//Dice agli altri thread di notificare ai loro client che la lista degli utenti online autenticati � cambiata
		
		if(listaThread.size() > 1) { // se c'� gi� un ServerThread oltre quello attuale
		...
		int count = 0; 		
	      while (count < listaThread.size()) {
		  		...
		  		if(listaThread.get(count).getSESSION().equals(SESSION)) {
					...
					for(ServerThread sthr : listaThread ) {	//dico ai ServerThread restanti che uno di loro � stato kickato, e quindi si aggiorna la lista online
						if(sthr.isAuth()) {
