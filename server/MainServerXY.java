...
public class MainServerXY implements Runnable{
	...
	public int addPanelThread(final ServerThread th, SocketAddress clientAddress) {
		...
		final JPanel panel = new JPanel(); 		 //panel � il pannello padre della GUI relativa ad un client
		final String session = th.getSESSION();
		panel.setLayout(new WrapLayout());
		panel.setName(session);
		panel.setSize(500, 90);
		
		JPanel topPanel = new JPanel();			//topPanel � il pannello posto superiormente
				...
				if(tmpTh instanceof ServerThread) {
					...
					try {
						...
						if(!PoolThread.deleteSocket((ServerThread)tmpTh, session)) 
							...
							log("Il socket: " + session + " � stato disconnesso");
						}
					} catch (Exception e1) {
	 ...
	 * quindi se si vuole aggiornare il JEditoPane, � utile inserire il testo da aggiornare prima di un tag (solitamente BODY) 
	 * 
	 */
	...
	public synchronized static boolean updateGameFlotta(Game game, Utente ut) {
		...
		System.out.println("Quantit� di game: ->"+listaGame.size());
					...
					if(tmpGame.getFlottaSfidato() != null) {
						...
						for(Quadrato tmpQuad : flotta) {
							if(tmpQuad.getName().equals(quad.getName())) {
								...
								//Comunico al serverThread il quadrato dell'avversario, in modo che poi al clientThread verr� notificato tale quadrato, per poter capire quale Nave ha colpito
	...
	public synchronized static void endGame(Utente vincitore, Utente perdente, Game game) {
		...
		//il perdente � stato gi� notificato dal suo serverThread
