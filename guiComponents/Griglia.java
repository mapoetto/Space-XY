...
public class Griglia extends JPanel implements MouseMotionListener, MouseListener{
	...
	public Griglia(MainFrame mf) {
		...
		unit� = this.mf.getDimensionRatio(50);
		...
		Runnable createGui = new Runnable() {
			public void run() {
				...
				while(xg <= (unit�*10)) {
					...
					while(yg <= (unit�*10)) {
						tmpLabel = new Quadrato(xg,yg,unit�,unit�, xg/unit�, yg/unit�);
				...
				while(yg <= (unit�*10)) {
					...
					tmpLabelCoord.setBounds(0, yg, unit�, unit�);
				...
				while(xg <= (unit�*10)) {
					...
					tmpLabelCoord.setBounds(xg, 0, unit�, unit�);
	...
	public void mouseMoved(MouseEvent e) {
		...
		if(mf.myData.game.getStato().equals("Iniziale")){
			...
			if(findComponentAt(e.getPoint()) instanceof Quadrato) {
				...
				//qdr � il quadrato su cui scorre il mouse
		...
		*/
		 ...
		 * poi sposto il cursore all'origine del quadrato pi� vicino, per aiutarlo ad inserirlo correttamente
		 * ovvero, prendi la posizione del mouse, trova lo spigolo in alto a sinistra pi� vicino e disegnalo a partire da la
		 * 
		 * 
		 * 
		 * draggedImg � una variabile TEMP che viene settata nella classe PaginaNavi, quando l'utente fa click su un oggetto Nave
		 * 
		 * 
		 */
		...
		//controlla navi che sono in "floating" cio� sono state selezionate senza essere posizionate
		checkFloatShips();
		...
		if(draggedImg != null) {
			...
			int numQuadratiLunghezza = (int)draggedImg.getWidth()/unit�;
			int numQuadratiAltezza = (int)(numQuadratiLunghezza * (draggedImg.getHeight()/unit�) - numQuadratiLunghezza);
			
			int numQuadratiTot = numQuadratiLunghezza + numQuadratiAltezza;
			
			//System.out.println("Lunghezza->:"+numQuadratiLunghezza+"esce da:"+draggedImg.getWidth()+"diviso"+unit�);
			
			if(shouldIdraw) {
				mayIwrite = true;
				
				
				//se la nave che si sta muovendo sulla griglia, era gi� posizionata, bisogna rimuovere i suoi quadratri precedentemente intersecati
				...
				//prendi la posizione del mouse, trova lo spigolo in alto a sinistra pi� vicino e disegnalo a partire da la
				...
				//questa lista servir� al metodo successivo per capire se pu� effettivamente disegnare quella nave, quindi qui lo puliamo se era stato gi� settato da altre navi
				...
				Point inAltoDestra = new Point(draggedImg.getX()+draggedImg.getWidth()-unit�, draggedImg.getY());
				Point inBassoDestra = new Point(draggedImg.getX()+draggedImg.getWidth()-unit�, draggedImg.getY()+draggedImg.getHeight()-unit�);
				Point inBassoSinistra = new Point(draggedImg.getX(), draggedImg.getY()+draggedImg.getHeight()-unit�);
	...
	public Rectangle getCornerQuadrato(String corner, ArrayList<Quadrato> listaQuadrati) {
		...
		if(corner.equals("in alto a destra")) {
			...
			tmpRect = new Rectangle(aDestra.getX(), inAlto.getY(), unit�, unit�); //se le coordinate della figura disegnata non sforano questo quadrato, allora vuol dire che quel lato � disegnato bene
			
		}else if(corner.equals("in alto a sinistra")) {
			
			tmpRect = new Rectangle(aSinistra.getX(), inAlto.getY(), unit�, unit�); //se le coordinate della figura disegnata non sforano questo quadrato, allora vuol dire che quel lato � disegnato bene
			
		}else if(corner.equals("in basso a destra")) {
			
			tmpRect = new Rectangle(aDestra.getX(), inBasso.getY(), unit�, unit�); //se le coordinate della figura disegnata non sforano questo quadrato, allora vuol dire che quel lato � disegnato bene
			
		}else if(corner.equals("in basso a sinistra")){
			
			tmpRect = new Rectangle(aSinistra.getX(), inBasso.getY(), unit�, unit�); //se le coordinate della figura disegnata non sforano questo quadrato, allora vuol dire che quel lato � disegnato bene
		...
		public void mouseClicked(MouseEvent e) {
			...
			if(draggedImg != null && mf.myData.game.getStato().equals("Iniziale")){
				if(shouldIdraw) {
					if(mayIwrite) {
						...
						//controlliamo se la nave era gi� inserita precedentemente, in tal caso la rimuoviamo
			...
			//se la partita � iniziata, ed � il mio turno
			if(mf.myData.game.getStato().equals("iniziato") && mf.myData.game.getTurno().getSESSION().equals(mf.myData.getMe().getSESSION())) {
				...
				if(quadFound != null) {
					...
					if(quadFound.isClickato() == false) {
						...
						stackQuad.add(quadFound); // ogni volta che clicko un quadrato lo aggiungo allo stack, cos� quando ricevo la risposta del server, so che quella risposta � inidirizzato all'ultimo elemento aggiunto allo stack (in modo da allegerire il pacchetto di risposta del server, poich� non mantiene informazioni riguardanti il quadrato specifico)
						
						//aggiorniamo il model dicendo che non � pi� il nostro turno
						...
						mf.updateLog(new LogMessage("Game","Hai gi� clickato questo quadrato",dtf.format(LocalTime.now())));
					}
				}else {
				...
				mf.updateLog(new LogMessage("Game","Stai premendo su un game non iniziato o non � il tuo turno",dtf.format(LocalTime.now())));
			}
		...
		public void mouseEntered(MouseEvent e) {
			// TODO Auto-generated method stub
			if(mf.myData.game.getTurno().getSESSION().equals(mf.myData.getMe().getSESSION()) && mf.myData.game.getStato().equals("iniziato")) { // se � il mio turno 
