...
public class Page extends JLayeredPane{
	...
	//Controlla se ci sono Button o CheckButton in movimento, in tal caso la pagina non � pronta per essere interagibile
	public void checkComponentsReady() {
		...
		while(indice < listaComponenti.size()) {
			...
			if(listaComponenti.get(indice) instanceof Button) {
				...
				if(!((Button)listaComponenti.get(indice)).interagibile && ((Button)listaComponenti.get(indice)).isVisible()) { //se � visibile ma non � interagibile vuol dire che si sta muovendo
	...
	public boolean entra() {
		...
		//se la pagina non � interagibile esci dal metodo con false
		if(!this.interagibile) {
			mf.updateLog(new LogMessage("System","Impossibile far entrare una nuova pagina poich� ci sono animazioni in corso",dtf.format(LocalTime.now())));
			return false;
		}
		...
		do {
			//System.out.println("Aspetto che i timer (FadeIn) finiscano. FADEIN � nell'EDT ? " + SwingUtilities.isEventDispatchThread());
	...
	public void esci(Page entraPage) { //la pagina che dovr� essere mostrata dopo l'uscita
		
		//se la pagina non � interagibile esci dal metodo
		if(!this.interagibile) {
			mf.updateLog(new LogMessage("System","Impossibile uscire dalla pagina poich� ci sono animazioni in corso",dtf.format(LocalTime.now())));
			return;
		}
		...
		do {
			//System.out.println("Aspetto che i timer (FadeOut) finiscano. FADEOUT � nell'EDT ? " + SwingUtilities.isEventDispatchThread());
		}while(timerRunning(tmpTimerList));
